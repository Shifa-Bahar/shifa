...
class PromotionsMultipleAdapter(context: Activity, private val onItemTapped: ClickHomeSubItem) :
  RecyclerView.Adapter<PromotionsMultipleAdapter.ItemViewHolder>() {
  var arrayList: ArrayList<SectionData>? = ArrayList()
  var activity: Activity = context


  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
    val binding: ItemPromoMutliptleBinding = DataBindingUtil.inflate(
      LayoutInflater.from(activity),
      R.layout.item_promo_mutliptle,
      parent, false
    )
  ...
  class ItemViewHolder internal constructor(itemView: View) :
    ...
    var binding: ItemPromoMutliptleBinding? = DataBindingUtil.bind(itemView)
  }
  ...
  override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
    ...
    holder.binding?.clMain?.layoutParams?.height = (withPers * 0.6).roundToInt()
